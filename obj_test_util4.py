...
def f(x):
    ...
    for i in range(0, 5):
        prod = prod * x[i] * (1 - x[i])
    return np.sqrt(np.log(prod + 1)) + p(x)
...
def hessian_term_f(x, j, k):
    if j != k:
        ...
        return ((1 - x[j]) * (term_1 + term_2 + term_3) / 2) + hessian_term_p(x, j, k)
    elif j == k:
        prod_1 = 1
        prod_2 = 1
        for i in range(0, 5):
        ...
        return (prod_1 * (term_1 + term_2 + term_3) / 2) + hessian_term_p(x, j, k)
